...
class OCRLib:
    ...
    def process_rgb_image(self, input_image, sampling_rate, pre_boxes, pre_results):
        input_image = self.selector.select(input_image)
        results = []
        cat_results = []
        boxes = []
        if len(input_image) != 0:  # When the image is not rejected
            boxes = self.detector.detect(input_image)
            if sampling_rate == 5:  # Previous image is clear text or medium blur text
                if len(boxes) == len(pre_boxes):
                    # Pass to recognizer
                    for box in boxes:
                        result = self.recognizer.recognize(input_image, box)
                        results.append(result)

                    # We get the correct clear text, increase the sampling rate to 120
                    if results.sort() == pre_results.sort():
                        sampling_rate = 120
                    # We may get partial of the clear text image, reduce the sampling rate to 5 to get another one
                    else:
                        pre_results_set = set(pre_results)
                        results_set = set(results)
                        cat_results = results + list(pre_results_set - results_set)
                        sampling_rate = 5
                else:  # May be the drone adjusting vision or leaving the current scene
                    for box in boxes:
                        result = self.recognizer.recognize(input_image, box)
                        results.append(result)
                    ...
                    # If cat_results matches pre_results, which means the poster view is decreasing, increase
                    # sample_rate Assume the possible for view to increase to partial and decrease is extremely low
                    if cat_results.sort() == pre_results.sort():
                        sampling_rate = 120
                    else:  # The view for poster is increasing
                        sampling_rate = 5
                    # Need to save the words content

            elif sampling_rate == 30:  # Initialize or previous SR is 90
                if len(boxes) > 0:  # detect something
                    for box in boxes:
                        result = self.recognizer.recognize(input_image, box)
                        results.append(result)
                    cat_results = results.copy()
                    sampling_rate = 5
                else:  # still nothing detected
                    sampling_rate = 30

            # When sampling_rate == 90 or 120, already got clear images of previous scene
            else:
                if len(boxes) > 0:  # detect something
                    for box in boxes:
                        result = self.recognizer.recognize(input_image, box)
        ...
        else:  # When the image is rejected, increase the sampling rate to 90
            if sampling_rate >= 90:
                sampling_rate = 30
            else:
                sampling_rate = 90

        return results, sampling_rate, cat_results, boxes

...
def process_video(input_video, cfg_fn, results_path):
    reader = OCRLib(cfg_fn)
    vidcap = cv2.VideoCapture(input_video)
#     vidcap = skvideo.io.vreader(input_video) 
    assert vidcap.isOpened()

    # Set the original sample rate to 30
    sampling_rate = 30
    # Set pre_boxes and pre_results to empty
    pre_boxes = []
    pre_results = []
...
#     for img in vidcap:
        ...
        results, sampling_rate, cat_results, boxes = reader.process_rgb_image(image, sampling_rate, pre_boxes, pre_results)
        # ============================================================================
        if len(results) == 0:
            continue
        # ============================================================================
        # Save the pre_bbox and pre_results when sampling_rate is small; which means the previous image is clear text
        if sampling_rate == 5:
            pre_boxes = boxes.copy()
            pre_results = cat_results.copy()
        else:
            pre_boxes = []
            pre_results = []

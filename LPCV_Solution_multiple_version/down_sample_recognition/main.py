...
from libs.utils import apply_canny, dsample_image, skip_images
from libs.utils import reject
...
#     for img in vidcap:
        ...
        # Apply histogram along x and y axis
        ...
        if reject(peaks_x, peaks_y, mean_x, mean_y):
            # Compute x and y coordinates for poster
            if skip_images(mean_x, mean_y):
                xmin, xmax, ymin, ymax = int(peaks_x[0]), int(peaks_x[-1]), int(peaks_y[0]), int(peaks_y[-1])
                # The skipped images (complicated case) should be output here
                # cv2.imwrite(os.path.join(save_dir, 'skip_' + filename), image[ymin:ymax, xmin:xmax])

...
class OCRLib:
    ...
    def process_rgb_image(self, input_image, sampling_rate, cache_boxes, cache_image):
        ...
        if len(input_image) != 0:
            ...
            if len(boxes) > 0:
                ...
                if sampling_rate == 10:
                    if len(cache_boxes) > len(boxes):
                        for box in cache_boxes:
                            result = self.recognizer.recognize(cache_image, box)
                    ...
                    cache_boxes = boxes.copy()
                    sampling_rate = 10
            else:  # Assume heavy blur images
                sampling_rate = 60
        else:  # non_text or highly blur
            sampling_rate = 90
        return results, sampling_rate, cache_boxes

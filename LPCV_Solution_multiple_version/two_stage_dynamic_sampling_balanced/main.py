...
def parse_args():
    ...
    parser.add_argument(
        "--sampling_rate", help="Initial sample rate", default=30, type=int
    )
    parser.add_argument(
...
def load_result_index(results_path):
...
def process_video(input_video, cfg_fn, results_path, init_sampling_rate):
    ...
    while success:
        ...
        results, state_flag, cache_boxes_list = reader.process_rgb_image(image, cache_boxes, cache_images, state_flag)
        if cache_boxes_list is not None:
            cache_images.append(image)
        else:
            cache_images.clear()

        # Update sampling_rate based on state_flag
        if state_flag == 'NT':
            sampling_rate = SAMPLE_RATE_DICT['NT']
        elif state_flag == 'HQ':
            sampling_rate = SAMPLE_RATE_DICT['HQ']
        else:
            sampling_rate = SAMPLE_RATE_DICT["LQ"]
...
def query_video(
    ...
    txt2frm, frm2txt = load_result_index(results_path)
    print('frm2txt')
    print(frm2txt)
    print('txt2frm')
    print(txt2frm)

    # Delete continues frame

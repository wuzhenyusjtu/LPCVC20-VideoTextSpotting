...
with open('config.json', "r") as h_config:
    config = json.load(h_config)

alphabet_list = pkl.load(open('models/alphabet.pkl', 'rb'))
...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    if len(fg_index) > config["RPN_POSITIVE_NUM"]:
        labels[np.random.choice(fg_index, len(fg_index) - config["RPN_POSITIVE_NUM"], replace=False)] = -1

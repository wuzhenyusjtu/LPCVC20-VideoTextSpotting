...
if __name__ == '__main__':
    ...
    print("Static Quantization for model.cnn")
    ...
    for idx, m in enumerate(model.rnn.children()):
        print('Dynamic Quantization for model.rnn')
        quantized_lstm = torch.quantization.quantize_dynamic(m, {torch.nn.LSTM, torch.nn.Linear}, dtype=torch.qint8)
        torch.jit.save(torch.jit.script(quantized_lstm), '{}/rnn_{}_CRNN_{}.torchscript'.format(opt.expr_dir, idx, opt.qconfig))

...
def test(net, images_folder, output_folder, scaled_height):
    ...
    for image_name in pbar:
        ...
        with open('{}'.format(os.path.join(output_folder, 'res_{}.txt'.format(prefix))), 'w') as f:
            if polys.shape[0] > 0:
                cv2.imwrite(os.path.join(images_folder, prefix) + '_1.jpg', image)
                for id in range(polys.shape[0]):
                    f.write('{}, {}, {}, {}, {}, {}, {}, {}\n'.format(
                        int(polys[id, 0] / scale_x), int(polys[id, 1] / scale_y), int(polys[id, 2] / scale_x),
                        int(polys[id, 3] / scale_y), int(polys[id, 4] / scale_x), int(polys[id, 5] / scale_y),
                        int(polys[id, 6] / scale_x), int(polys[id, 7] / scale_y)
                    ))
            else:
                cv2.imwrite(os.path.join(images_folder, prefix) + '_0.jpg', image)

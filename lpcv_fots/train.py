...
#     checkppoint_name = os.path.join(folder, 'epoch_8_checkpoint.pt')
    checkppoint_name = os.path.join(folder, 'epoch_582_checkpoint.pt')
...
def fit(start_epoch, model, loss_func, opt, lr_scheduler, best_score, max_batches_per_iter_cnt, checkpoint_dir, train_dl, valid_dl):
    ...
    for epoch in range(start_epoch, 582+15):
        model.train()
...
if __name__ == '__main__':
    ...
    data_set = datasets.SynthText(args.train_folder, datasets.transform_finetune)
    # data_set = datasets.ICDAR2015(args.train_folder, datasets.transform)
#     data_set = datasets.SampleDataset(args.train_folder, datasets.transform_finetune)
    ...
    checkoint_dir = '/data/yunhe/fots_model/syn_full_270'

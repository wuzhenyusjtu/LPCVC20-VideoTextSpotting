...
def fit(start_epoch, num_epochs, model, loss_func, opt, lr_scheduler, best_score, max_batches_per_iter_cnt,
        checkpoint_dir, train_loader, val_loader):
    for epoch in range(start_epoch, start_epoch + num_epochs):
        val_loss = train_one_epoch(model, loss_func, opt, lr_scheduler, max_batches_per_iter_cnt, train_loader,
                                   val_loader, epoch)
...
if __name__ == '__main__':
    ...
    if args.train_folder_sample:
        # Merge SynthText dataset with sample dataset we generated from LPCV videos, use merged dataset for training
        # and testing
        ...
        if args.val:
            ...
            data_set_val = datasets.MergeText(args.train_folder_syn, args.train_folder_sample, datasets.transform,
                                              train=False)
    ...
    epoch, model, optimizer, lr_scheduler, best_score = restore_checkpoint(args.continue_training, args.pretrain_model,
                                                                           args.prune)
    if args.ngpus > 1:
        print('Use parallel')
        model = torch.nn.DataParallel(model)
    fit(epoch, args.epochs, model, detection_loss, optimizer, lr_scheduler, best_score, args.batches_before_train,
        checkpoint_dir, train_loader, val_loader)

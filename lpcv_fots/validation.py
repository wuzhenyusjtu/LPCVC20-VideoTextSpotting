...
def val_one_epoch(model, loss_func, opt, max_batches_per_iter_cnt, val_loader):
    ...
    with torch.no_grad():
        ...
        for cropped, classification, regression, thetas, training_mask in pbar:
            ...
            if prediction[0].shape[-1] != classification.shape[-1] or prediction[0].shape[-2] != classification.shape[
                -2]:
                cnt_error += 1
                continue
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test-folder-sample', type=str, required=True,
                        help='Path to folder with sample test images and labels')
    ...
    # Get dataloaders
    data_set_val = datasets.ICDAR2013(args.test_folder_sample, datasets.transform, train=False)
    val_loader = torch.utils.data.DataLoader(data_set_val, batch_size=1, shuffle=False,

...
Use ```export_pruned_fots.py``` to export actual pruned model from pruned masked model. After we export the model, we finetune it with Merged Dataset.

```sh
 python3 export_pruned_fots.py --save-dir /path/to/save/dir --prune-model /path/to/nni/pruned/model
```

...
def transform(im, quads, texts, normalizer, data_set, AUG=True, RESHAPE_SIZE=290, IN_OUT_RATIO=4, IN_SIDE=280,
              ANGLE=0, STRETCH=0, img_path=None):
    """
    ...
    assert 0 <= STRETCH < 1, 'STRETCH should be in range (0, 1)'
    assert RESHAPE_SIZE * (1 - STRETCH) >= IN_SIDE, 'STRETCH parameter too large, try with a smaller one'
    assert 45 >= ANGLE >= 0, 'ANGLE should be in range (0, 45)'
    ...
    if AUG:
        ...
        # grab the rotation matrix (applying the negative of the angle to rotate clockwise), then grab the sine and
        # cosine
        ...
        if 0 != crop_max_y:
            crop_point = (torch.randint(low=0, high=stretched.shape[1] // IN_OUT_RATIO - OUT_SIDE, size=(1,),
                                        dtype=torch.int16).item(),
                          torch.randint(low=0, high=stretched.shape[0] // IN_OUT_RATIO - OUT_SIDE, size=(1,),
                                        dtype=torch.int16).item())
        else:
            crop_point = (torch.randint(low=0, high=stretched.shape[1] // IN_OUT_RATIO - OUT_SIDE, size=(1,),
                                        dtype=torch.int16).item(),
    ...
    for quad_id, quad in enumerate(quads):
        ...
        if type(intersected_polygon) is Polygon:
            ...
            shrunk_width_and_height = (intersected_minAreaRect[1][0] - shrinkage,
                                       intersected_minAreaRect[1][1] - shrinkage)
            if shrunk_width_and_height[0] >= 0 and shrunk_width_and_height[1] >= 0 and texts[quad_id]:
                ...
                cv2.fillConvexPoly(tmp_cls, round_shrink_minAreaRect_boxPoints.round(
                    out=round_shrink_minAreaRect_boxPoints).astype(int), 1)
                cv2.rectangle(tmp_cls, (0, 0), (tmp_cls.shape[1] - 1, tmp_cls.shape[0] - 1), 0,
                              thickness=int(round(shrinkage * 2)))
                ...
                for point in pointsT:
                    for plane in range(3):  # TODO width - dist, height - other dist and more precise dist
                        regression[(plane,) + tuple(point)] = point_dist_to_line(poly[plane], poly[plane + 1], np.array(
                            (point[1], point[0]))) * IN_OUT_RATIO
                    regression[(3,) + tuple(point)] = point_dist_to_line(poly[3], poly[0],
                                                                         np.array((point[1], point[0]))) * IN_OUT_RATIO
    ...
    if AUG:
        cropped = stretched[crop_point[1] * IN_OUT_RATIO:crop_point[1] * IN_OUT_RATIO + IN_SIDE,
                  crop_point[0] * IN_OUT_RATIO:crop_point[0] * IN_OUT_RATIO + IN_SIDE]
    else:
        cropped = stretched[crop_point[1] * IN_OUT_RATIO:crop_point[1] * IN_OUT_RATIO + IN_SIDE_X,
                  crop_point[0] * IN_OUT_RATIO:crop_point[0] * IN_OUT_RATIO + IN_SIDE_Y]
...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(self, root, transform, train=True):
        ...
        self.image_prefix = os.listdir(os.path.join(root, self.img_dir))
        self.transform = transform
    ...
    def __getitem__(self, idx):
        img = cv2.imread(os.path.join(os.path.join(self.root, self.img_dir), self.image_prefix[idx]),
                         cv2.IMREAD_COLOR).astype(np.float32)
        # quads = []
        # texts = []

        lines = [line.rstrip('\n') for line in
                 open(os.path.join(os.path.join(self.root, self.labels_dir),
                                   self.image_prefix[idx].replace('.jpg', '.txt')),
                      encoding='utf-8-sig')]
        quads = np.zeros((len(lines), 4, 2))
        texts = np.zeros(len(lines))
        for i in range(0, len(lines)):
            matches = lines[i].split(',')
            numbers = np.array(matches[:8], dtype=float)
            # quads.append(numbers.reshape((4, 2)))
            # texts.append('###' != matches[8])
            quads[i] = numbers.reshape((4, 2))
            texts[i] = ('###' != matches[8])
        if len(quads) == 0:
        ...
        if self.train:
            return self.transform(img, quads, texts.tolist(), self.normalizer, self, AUG=True, RESHAPE_SIZE=320,
                                  IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=10, STRETCH=0.1)
        else:
            return self.transform(img, quads, texts.tolist(), self.normalizer, self, AUG=False, RESHAPE_SIZE=280,
                                  IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=0, STRETCH=0)
...
class MergeText(torch.utils.data.Dataset):
    def __init__(self, root_syn, root_sample, transform, train=True):
        ...
        self.sample_image_prefix = os.listdir(os.path.join(self.sample_root, self.sample_img_dir))
    ...
    def __getitem__(self, idx):
        ...
        if idx < self.syn_len:
            ...
            img = cv2.imread(os.path.join(self.syn_root, self.syn_labels['imnames'][0, idx][0]),
                             cv2.IMREAD_COLOR).astype(np.float32)
            ...
            if (transposed > 0).all() and (transposed[:, :, 1] < img.shape[1]).all() and (
                    transposed[:, :, 1] < img.shape[0]).all():
                if ((transposed[:, 0] != transposed[:, 1]).all() and
                        ...
                        (transposed[:, 2] != transposed[:, 3]).all()):  # boxes can be in a form [p1, p1, p2, p2],
                    # while we need [p1, p2, p3, p4]
                    # Transformation parameters can be set here
                    if self.train:
                        return self.transform(img, transposed, (True,) * len(transposed), self.normalizer, self,
                                              AUG=True, RESHAPE_SIZE=320, IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=10,
                                              STRETCH=0.1)
                    else:
                        return self.transform(img, transposed, (True,) * len(transposed), self.normalizer, self,
                                              AUG=False, RESHAPE_SIZE=280, IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=0,
                                              STRETCH=0)
            self.syn_broken_image_ids.add(idx)
            return self[torch.randint(low=0, high=self.syn_len, size=(1,), dtype=torch.int16).item()]
        else:
            # with CodeTimer("Load img and labels"):
            img = cv2.imread(os.path.join(os.path.join(self.sample_root, self.sample_img_dir),
                                          self.sample_image_prefix[idx - self.syn_len]),
                             cv2.IMREAD_COLOR).astype(np.float32)
            img_path = os.path.join(os.path.join(self.sample_root, self.sample_img_dir),
                                    self.sample_image_prefix[idx - self.syn_len])

            lines = [line.rstrip('\n') for line in open(
                os.path.join(os.path.join(self.sample_root, self.sample_labels_dir),
                             self.sample_image_prefix[idx - self.syn_len].replace('.jpg', '.txt')), encoding='utf-8-sig')]
            quads = np.zeros((len(lines), 4, 2))
            texts = np.zeros(len(lines))
            for i in range(0, len(lines)):
                matches = lines[i].split(',')
            ...
            if self.train:
                return self.transform(img, quads, texts.tolist(), self.normalizer, self, AUG=True, RESHAPE_SIZE=320,
                                      IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=10, STRETCH=0.1, img_path=img_path)
            else:
                return self.transform(img, quads, texts.tolist(), self.normalizer, self, AUG=False, RESHAPE_SIZE=280,
                                      IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=0, STRETCH=0, img_path=img_path)
...
class SynthText(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.broken_image_ids:
            return self[torch.randint(low=0, high=len(self), size=(1,), dtype=torch.int16).item()]
        img = cv2.imread(os.path.join(self.root, self.labels['imnames'][0, idx][0]), cv2.IMREAD_COLOR).astype(
            np.float32)
        if 190 >= img.shape[0]:  # image is too low, it will not be possible to crop 640x640 after transformations
        ...
        if (transposed > 0).all() and (transposed[:, :, 1] < img.shape[1]).all() and (
            ...
            if ((transposed[:, 0] != transposed[:, 1]).all() and
                    ...
                    (transposed[:, 1] != transposed[:, 2]).all() and
                    (transposed[:, 1] != transposed[:, 3]).all() and
                    (transposed[:, 2] != transposed[:, 3]).all()):  # boxes can be in a form [p1, p1, p2, p2], while
                # we need [p1, p2, p3, p4]

                # Transformation parameters can be set here
                if self.train:
                    return self.transform(img, transposed, (True,) * len(transposed), self.normalizer, self, AUG=True,
                                          RESHAPE_SIZE=320, IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=10, STRETCH=0.1)
                else:
                    return self.transform(img, transposed, (True,) * len(transposed), self.normalizer, self, AUG=False,
                                          RESHAPE_SIZE=280, IN_OUT_RATIO=4, IN_SIDE=280, ANGLE=0, STRETCH=0)
        self.broken_image_ids.add(idx)
        return self[torch.randint(low=0, high=len(self), size=(1,), dtype=torch.int16).item()]
